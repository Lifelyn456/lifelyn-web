...
export function WalletLogin({ register = false }: { register?: boolean }) {
  ...
  async function connect() {
    ...
    setBusy(true);
    setError("");
    setStatus("Looking for Freighter…");
    try {
      if (!(await isConnected()).isConnected)
        throw new Error(
          "Freighter was not found. Install and unlock the official Freighter extension, then try again.",
        );
      ...
      const challenge = validateChallenge(
        await apiRequest<unknown>("/auth/challenge", {
          method: "POST",
          body: JSON.stringify({ address: access.address }),
        }),
        access.address,
      ...
      if (signed.error || !signed.signedMessage || signed.signerAddress !== access.address)
        throw new Error("The selected Freighter wallet did not sign the exact challenge.");
      ...
      const account = register
        ? await apiRequest<Account>("/me", {
            ...
            body: JSON.stringify({
              ...
              ...(role === "CLINICIAN" ? { providerType: providerType.trim() } : {}),
            }),
          })
        : await apiRequest<Account>("/me");
      router.replace(
  ...
  return (
    <div className="auth-layout">
      <section className="auth-story">
        ...
        <p>Your continuous, patient-owned health memory.</p>
      ...
      <main className="auth-content">
        <div className="auth-card">
          ...
          {register && (
            <>
              <div className="auth-role-options" role="group" aria-label="Account type">
                ...
                >
                  <User size={17} /> Patient
                </button>
                <button
                  className={role === "CLINICIAN" ? "selected" : ""}
                  aria-pressed={role === "CLINICIAN"}
                  onClick={() => setRole("CLINICIAN")}
                >
                  <Stethoscope size={17} /> Clinician
                </button>
              </div>
          ...
          <button className="button" onClick={connect} disabled={busy}>
            <Wallet size={19} />
            {busy ? "Working…" : register ? "Register with Freighter" : "Sign in with Freighter"}
            <ArrowUpRight size={17} />
          </button>
          <div className="wallet-note">
            <ShieldCheck size={18} />
            <span>
              The signed message proves wallet ownership only. No transaction or payment is created.
            </span>
          </div>
          <div className="auth-divider" />
          <p>{register ? "Already registered?" : "New to Lifelyn?"}</p>
          <Link href={register ? "/login" : "/register"} className="text-button">
            {register ? "Sign in" : "Create an account"}
            <ArrowUpRight size={16} />
          </Link>
          <Link className="auth-back" href="/">
            <ArrowLeft size={14} /> Back to Lifelyn
          </Link>
        </div>
      </main>
    </div>
