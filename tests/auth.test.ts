...
it("rejects origin substitution, wallet substitution and altered messages", () => {
  expect(() => validateChallenge(challenge, fields.address, fields.origin, now)).not.toThrow();
  expect(() => validateChallenge(challenge, fields.address, "https://evil.example", now)).toThrow();
  expect(() => validateChallenge(challenge, "another-address", fields.origin, now)).toThrow();
  expect(() =>
    validateChallenge(
...
it("rejects expired challenges", () =>
  expect(() =>
    validateChallenge(challenge, fields.address, fields.origin, Date.parse(fields.expiresAt)),
  ).toThrow());
